from ml.stockgan import Generator, Discriminator
import torch
...
class Env():
    def __init__(self):
        ...
        data = np.array([data['Open'], data['High'],
                        data['Low'], data['Close']])
        data = data.transpose()
        self.batch_size = 64
    ...
    def build_model(self):
        self.D = Discriminator(self.input_size)
    ...
    def train(self):
        ...
        for episode in tqdm(range(numEpisodes)):
            ...
            real_input = torch.from_numpy(
                self.data_passer.next_batch().transpose()).float()
            ...
            # transpose fake_input
            # fake_input = fake_input.transpose(1, 2)
