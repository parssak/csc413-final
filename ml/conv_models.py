...
class TemporalBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        print(out.shape, res.shape, self.downsample)
...
class CausalConvGenerator(nn.Module):
    ...
    def __init__(
        self, noise_size, output_size, n_layers, n_channel, kernel_size, dropout=0
    ):
        super().__init__()
        num_channels = [n_channel] * n_layers
        self.tcn = TCN(noise_size, output_size,
...
if __name__ == "__main__":
    ...
    gen = CausalConvGenerator(
        noise_size=30, output_size=1, n_layers=8, n_channel=10, kernel_size=8, dropout=0
    )
    dis = CausalConvDiscriminator(
