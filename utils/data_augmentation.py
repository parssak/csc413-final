...
class DataAugmentation:
    ...
    def stretching(self, tradingEnv, factor=1):
        ...
        for i in range(1, len(newTradingEnv.data.index)):
            newTradingEnv.data['Close'][i] = newTradingEnv.data['Close'][i -
                                                                         1] * (1 + returns[i])
            newTradingEnv.data['Low'][i] = newTradingEnv.data['Close'][i] * \
                tradingEnv.data['Low'][i]/tradingEnv.data['Close'][i]
            newTradingEnv.data['High'][i] = newTradingEnv.data['Close'][i] * \
                tradingEnv.data['High'][i]/tradingEnv.data['Close'][i]
    ...
    def lowPassFilter(self, tradingEnv, order=5):
        ...
        newTradingEnv.data['Close'] = newTradingEnv.data['Close'].rolling(
            window=order).mean()
        newTradingEnv.data['Low'] = newTradingEnv.data['Low'].rolling(
            window=order).mean()
        newTradingEnv.data['High'] = newTradingEnv.data['High'].rolling(
            window=order).mean()
        newTradingEnv.data['Volume'] = newTradingEnv.data['Volume'].rolling(
            window=order).mean()
    ...
    def generate(self, tradingEnv):
        ...
        tradingEnvList = []
        shiftRange = constants['dataAugmentation']['shiftRanges']
        stretchRange = constants['dataAugmentation']['stretchRanges']
        filterRange = constants['dataAugmentation']['filterRanges']
        noiseRange = constants['dataAugmentation']['noiseRanges']
        for shift in shiftRange:
            tradingEnvShifted = self.shiftTimeSeries(tradingEnv, shift)
            for stretch in stretchRange:
                tradingEnvStretched = self.stretching(
                    tradingEnvShifted, stretch)
                for order in filterRange:
                    tradingEnvFiltered = self.lowPassFilter(
                        tradingEnvStretched, order)
