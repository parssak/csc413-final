...
class PerformanceEstimator:
    ...
    def computePnL(self):
        # Compute the PnL
        self.PnL = self.data["Money"].tolist()[-1] - self.data["Money"][0]
        return self.PnL
    ...
    def computeAnnualizedReturn(self):
        ...
        cumulativeReturn = cumulativeReturn.tolist()[-1]
        ...
        timeElapsed = len(self.data['Returns'])
        # Compute the Annualized Return
        if(cumulativeReturn > -1):
            self.annualizedReturn = 100 * \
                (((1 + cumulativeReturn) ** (365 * 1/timeElapsed)) - 1)
        else:
            self.annualizedReturn = -100
    ...
    def computeAnnualizedVolatility(self):
        ...
        self.annualizedVolatily = 100 * \
            np.sqrt(252) * self.data['Returns'].std()
    ...
    def computeSharpeRatio(self, riskFreeRate=0):
        ...
        if expectedReturn != 0 and volatility != 0:
            self.sharpeRatio = np.sqrt(
                252) * (expectedReturn - riskFreeRate)/volatility
        else:
            self.sharpeRatio = 0
    ...
    def computeProfitability(self):
        ...
        profit = 0
        loss = 0
        index = next((i for i in range(len(self.data.index))
                     if self.data['Action'][i] != 0), None)
        if index == None:
        ...
        delta = self.data['Money'].tolist()[-1] - money
        if(delta >= 0):
            good += 1
            profit += delta
        else:
    ...
    def computePerformance(self):
        ...
        self.performanceTable = [["Profit & Loss (P&L)", "{0:.0f}".format(self.PnL)],
                                 ["Annualized Return", "{0:.2f}".format(
                                     self.annualizedReturn) + '%'],
                                 ["Annualized Volatility", "{0:.2f}".format(
                                     self.annualizedVolatily) + '%'],
                                 ["Sharpe Ratio", "{0:.3f}".format(
                                     self.sharpeRatio)],
                                 ["Sortino Ratio", "{0:.3f}".format(
                                     self.sortinoRatio)],
                                 ["Maximum Drawdown", "{0:.2f}".format(
                                     self.maxDD) + '%'],
                                 ["Maximum Drawdown Duration",
                                     "{0:.0f}".format(self.maxDDD) + ' days'],
                                 ["Profitability", "{0:.2f}".format(
                                     self.profitability) + '%'],
        ...
        return {
            ...
            'MaxDrawdown': self.maxDD,
            'MaxDrawdownDuration': int(self.maxDDD),
            'Profitability': self.profitability,
            'AverageProfitLossRatio': 'inf' if math.isinf(self.averageProfitLossRatio) else self.averageProfitLossRatio,
    ...
    def displayPerformance(self, name):
        ...
        tabulation = tabulate(self.performanceTable, headers,
